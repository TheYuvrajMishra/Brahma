    ...
    const handleGoogleConnect = async () => {
        ...
        try {
            ...
            if (data.url) {
                ...
                // Polling to detect manual popup close
    ...
    return (
        <div className="min-h-screen w-full bg-[#09090b] text-white flex flex-col items-center justify-center relative overflow-hidden font-sans">
            ...
            <motion.div 
                ...
                className="w-full max-w-md mx-4 p-8 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-2xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col items-center text-center relative z-10"
            >
                {/* Logo & Header */}
                <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 shadow-inner">
                    <PiTerminalLight className="w-7 h-7 text-emerald-400" />
                </div>
                ...
                {/* Google Workspace Connection Pill — Reusing exact sidebar design pattern */}
                <div className="w-full p-4 rounded-2xl bg-white/[0.03] border border-white/10 flex flex-col gap-3 hover:border-white/20 transition-all duration-300">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                            ...
                            <span className="text-sm font-medium text-white/90">Google Account</span>
                        </div>
                        <span className="w-2.5 h-2.5 rounded-full bg-amber-500/60 shadow-[0_0_8px_rgba(245,158,11,0.5)]" />
                    </div>

                    <button
                        onClick={handleGoogleConnect}
                        disabled={connecting}
                        className="w-full py-3 px-4 rounded-xl bg-white/10 hover:bg-white/15 active:scale-[0.98] border border-white/20 text-xs font-semibold text-white tracking-wide flex items-center justify-center gap-2 transition-all duration-300 shadow-lg cursor-pointer disabled:opacity-50"
                    >
                        {connecting ? (
